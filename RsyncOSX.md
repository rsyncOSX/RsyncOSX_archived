...
The work on **version 6.6.8** is commenced. The major part in this release are internal changes regarding reading logrecords into memory, cleanup in code and so on. There might be several hundreds or thousends of logrecords and in the new version logrecords are not read from storage and loaded until needed.  There are also some minor cleanups in the UI.
