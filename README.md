...
This repository is updated for **Xcode 10.2** and **Swift 5**. Both are still in beta and yet not released by Apple. The branch `v5.7.1-fixes` is for updating fixes to the released version of RsyncOSX if the upcoming versions of Xcode 10.2 and Swift 5 takes some time to release.

The master repository is also updated with changes for next version and might be unstable. The branch `v5.7.1-fixes` is stable and will compile with released Xcode 10.1 and Swift 4.
